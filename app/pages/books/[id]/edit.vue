...
import type { Category, BookWithCategories } from '~/types'
...
async function onSubmit(event: FormSubmitEvent<Schema>) {
  ...
  try {
    ...
    if (res.success) {
      ...
      navigateTo(`/books/${route.params.id}`)
    }
  } catch (error: any) {
    toast.add({
      title: 'Error',
    ...
    <header class="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
      <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        <div class="flex items-center gap-4">
          <UButton :to="`/books/${route.params.id}`" variant="ghost" icon="i-lucide-arrow-left" size="sm">
            Back
          </UButton>
          ...
          <UFormGroup label="Rating" name="rating" description="Rate from 1 to 5 stars">
            <div class="flex gap-2">
              <UButton
                ...
                :color="state.rating && i <= state.rating ? 'yellow' : 'gray'"
                size="sm"
                icon="i-lucide-star"
